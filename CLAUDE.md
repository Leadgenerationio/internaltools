...
- **Longform video ads**: Multi-step pipeline: Brief → Script Gen (Claude) → Voiceover (ElevenLabs TTS) → B-Roll (kie.ai) → Stitch (FFmpeg). Captions are deferred — not applied during initial stitch, only added during reassemble/finalize via Submagic (the sole caption provider; built-in FFmpeg captioning removed). Per-scene regeneration supported with parallel processing and per-scene model selection. 5-step wizard at `/create/longform-video`. BullMQ worker (concurrency: 1). Token pricing: 35 tokens/variant with b-roll, 10 tokens/variant without, script generation FREE. Env vars: `ELEVENLABS_API_KEY`, `SUBMAGIC_API_KEY`.
...
- **App flow (Longform Video)**: `/create/longform-video` — 5-step wizard: Brief → Script Gen (Claude) → Voiceover (ElevenLabs) → B-Roll (kie.ai, per-scene model selection, parallel regeneration) → Stitch (FFmpeg). Captions deferred to reassemble/finalize step via Submagic. Background job via BullMQ longform queue.
