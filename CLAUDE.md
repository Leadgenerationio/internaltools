...
- **AI video generation**: kie.ai REST API — 6 models across 2 API patterns: Veo (veo3_fast, veo3) via `/veo/` endpoints, Market (seedance-1.5, kling-2.6, sora-2, sora-2-pro) via `/jobs/` endpoints. Per-model token pricing (3-25 tokens). Automatic model fallback on capacity errors: veo3_fast → seedance-1.5-pro → kling-2.6 (implemented in `src/lib/kie-api.ts`).
- **AI image generation**: kie.ai REST API (Nano Banana 2) for avatar images and product angle grids. Implemented in `src/lib/kie-image-api.ts`. Uses `/jobs/createTask` + `/jobs/recordInfo` polling pattern (5s interval, 3-min timeout).
- **Avatar video creator**: 6-step wizard at `/create/avatar-video`. Lip-sync video via Creatify Aurora API (`POST /api/aurora/`, poll `GET /api/aurora/{id}/`). No audio duration limit. Env vars: `CREATIFY_API_ID`, `CREATIFY_API_KEY`. Database schema (`Avatar` model), types (`src/lib/avatar-types.ts`), token pricing (AVATAR_IMAGE: 2, AVATAR_PRODUCT_GRID: 5, AVATAR_VOICEOVER: 2, AVATAR_VIDEO: 10), job types (`AvatarImageGenData`, `AvatarProductGridData`, `AvatarVideoGenData`), BullMQ `avatar` queue.
- **Longform video ads**: Redesigned 6-step wizard at `/create/longform-video`: Prompt → Voice & Edit → Build Scenes → Music → Captions → Finalize. Single freeform AI prompt generates N unique scene-aware scripts (Claude, FREE). Per-script voice selection + voiceover generation (ElevenLabs TTS, 2 tokens/script). Per-scene video building: AI generate (kie.ai, model-specific tokens), upload, or media library (FREE). Background music via Jamendo. Captions via Submagic. Aspect ratio selection (9:16, 16:9, 1:1). Final assembly via BullMQ `longform-finalize` job (FREE). Components decomposed into `src/components/longform/` (9 files). Pay-as-you-go token billing replaces flat per-variant pricing. Legacy pipeline still supported. Env vars: `ELEVENLABS_API_KEY`, `SUBMAGIC_API_KEY`.
...
- **Stock video**: Storyblocks API v2 integration via `src/lib/storyblocks.ts` — HMAC-SHA256 auth, search + download. "Stock Video" tab in VideoSourceTabs (text overlay) and SceneSlot (longform scenes). Downloads stock footage to local uploads + saves to media library. API routes at `/api/storyblocks/search` and `/api/storyblocks/download`. `StoryblocksSearch` reusable component with debounced search, video preview on hover, and inline download. Env vars: `STORYBLOCKS_PUBLIC_KEY`, `STORYBLOCKS_PRIVATE_KEY`.
...
- **Path utilities**: `src/lib/path-utils.ts` — shared `extractPublicPath()` function that converts any URL format (CDN, S3, Supabase, `/api/files`, relative) to a public-relative file path. Consolidates 4 previously duplicated implementations across the codebase.
...
- **Cloud storage**: Optional S3/R2/Supabase via `src/lib/storage.ts` — activated by `S3_BUCKET` env var, lazy-loads AWS SDK, streaming uploads via `@aws-sdk/lib-storage` (no full-file buffering). Supabase S3 compatibility supported (`forcePathStyle: true`, `S3_REGION` env var).
...
- **Auth**: NextAuth v5 credentials provider — JWT sessions, `AUTH_SECRET` env var for signing, `secureCookie: true` in middleware for reverse proxy compatibility. Service-to-service auth: middleware accepts `Authorization: Bearer AUTH_SECRET` for internal worker → web app calls.
...
- **Home page**: `/` is the ad creation model hub — grid of ad format cards (Video Text Overlay, Longform Video, and Video Cut Up are live, others coming soon). Requires auth, redirects to `/welcome` if unauthenticated.
- **App flow (Video Cut Up)**: `/create/video-cutup` — 3-step wizard: Upload → Review Clips → Done. FFmpeg scene detection auto-splits video into clips. User selects which clips to save to media library. FREE (no tokens). API routes at `/api/video-cutup/analyze` (scene detection) and `/api/video-cutup/split` (clip extraction + media library save). Core logic in `src/lib/scene-detect.ts`.
...
- **App flow (Longform Video)**: `/create/longform-video` — 6-step wizard: Prompt → Voice & Edit → Build Scenes → Music → Captions → Finalize. Freeform prompt → scene-aware scripts (Claude, FREE). Per-script voice + voiceover (ElevenLabs, 2 tok). Per-scene video via AI/upload/library. Music (Jamendo). Captions (Submagic). Aspect ratio (9:16/16:9/1:1). Final assembly via BullMQ `longform-finalize` (FREE). State persisted in localStorage.
- **App flow (Avatar Video)**: `/create/avatar-video` — 6-step wizard: Create Avatar → Edit & Save → Product (optional) → Script & Voice → Generate Video → Done. AI avatar image generation (Nano Banana 2, 2 tok). Save avatars to company library for reuse. ElevenLabs TTS voiceover (2 tok). Lip-sync video generation via Creatify Aurora API + BullMQ `avatar` queue (10 tok). API routes at `/api/avatar/*`. Types in `src/lib/avatar-types.ts`. Image API in `src/lib/kie-image-api.ts`. Worker in `src/workers/avatar-worker.ts`.
- **Overlay rendering**: Canvas PNG approach (not FFmpeg drawtext) for emoji support. Canvas dimensions validated (>0, <=4096px) before creation.
...
- **Render error reporting**: Both sync and worker render responses include an `errors[]` array with per-item error details. FFmpeg renderer validates output file exists and size > 1000 bytes. UI shows specific error messages to user.
...
- **Background jobs**: BullMQ + Redis for async render, video generation, and longform pipeline. Routes enqueue jobs and return `{ jobId }` immediately. Client polls `/api/jobs/[id]` with exponential backoff (3s→15s, render/video-gen max 20min, longform max 45min). Workers run as separate Railway service (`WORKER_MODE=true`). Graceful degradation: falls back to synchronous when `REDIS_URL` not set.
...
- **Stress Test Agent**: `npm run stress-test` — simulates N concurrent users (default 100) performing realistic flows: register, login, upload, render. Reports per-endpoint metrics (p50/p95/p99 latency, error rate, throughput). Config in `scripts/stress-test.config.json`. Env vars: `STRESS_BASE_URL`, `STRESS_USERS`, `STRESS_CONCURRENCY`, `STRESS_GENERATE_ADS`.
...
- **In-app notifications**: `NotificationBell` component in header — bell icon with unread badge, dropdown with recent notifications. Uses SSE (`/api/notifications/stream`) for real-time push via Redis pub/sub, falls back to 30s polling. API routes: `/api/notifications` (list), `/api/notifications/[id]/read`, `/api/notifications/mark-all-read`, `/api/notifications/stream` (SSE). `Notification` model in Prisma.
- **Email queue**: `src/lib/email-queue.ts` routes emails through BullMQ with 3 retries + exponential backoff. `src/workers/email-worker.ts` processes jobs (concurrency 5, rate-limited 20/10s). Falls back to direct send without Redis.
- **DB archival**: `/api/admin/cleanup` POST also archives old DB records (ApiUsageLog >90d, read Notifications >30d, ProcessedWebhookEvent >7d). Supports `CRON_SECRET` header for Railway cron service.
- **Auto-save wizard state**: When working on a saved project, debounced save to `/api/projects/[id]` every 30s when state changes. localStorage remains the primary backup.
