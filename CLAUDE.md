...
- **Avatar video creator** (Phase 1 — foundation): Database schema (`Avatar` model), types (`src/lib/avatar-types.ts`), token pricing (AVATAR_IMAGE: 2, AVATAR_PRODUCT_GRID: 5, AVATAR_VOICEOVER: 2, AVATAR_VIDEO: 10), job types (`AvatarImageGenData`, `AvatarProductGridData`, `AvatarVideoGenData`), BullMQ `avatar` queue. Components/pages not yet implemented.
